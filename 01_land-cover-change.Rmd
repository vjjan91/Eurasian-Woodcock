...
# Habitat loss of the Eurasian Woodcock

In this script, I adapting code that I wrote for another study examining 150 years of landscape and climate change on bird species of the Nilgiri hills. 

To assess habitat loss for the Eurasian Woodcock across the Nilgiri hills, we examine changes in marshland, swamps and grassland habitat over time. To do so, we will process land cover polygons and rasters for multiple time periods (1848 and 2018) across the Nilgiri hills of the Western Ghats biodiversity hotspot. For the year 1848, a survey map (created by Captain John Ouchterlony) was obtained from the British Library and the Tamil Nadu State Archive and was manually digitized to arrive at multiple land cover classes (for more information on the process of digitization, please read a forthcoming manuscript on landscape change). For the year 2018, we relied on satellite imagery from Sentinel-2 (the imagery was classified using Google Earth Engine and more information on the classification can be obtained below).
...
plot(rast1848, col = colors1848, main = "1848",
     legend = FALSE)
plot(rast2018_reclassified, col=colors2018, main = "2018", 
     legend=FALSE)
...
legend("bottom",
       legend=c("agriculture", "plantations",
                "settlements", "shola_forest",
                "shola_grassland", "swamps",
                "water_bodies"), fill=colors1848, 
       cex = 0.8, 
...
![A comparison of land cover classes between 1848 and 2018 revealed a 60% reduction in grassland and marshland over time](figs/fig_landCover_1848_vs_2018.png)
